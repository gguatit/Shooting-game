...
- **레벨업 시스템** - 탄약 소진 시 무기 선택으로 레벨업, 레벨당 예비 탄약 +30 보너스

### 무기 라인업 (12종)

#### 기본 무기 (게임 시작 시)
1. **AR-47 Phantom** - 균형잡힌 돌격 소총 (30발/90예비탄)

#### 첫 레벨업 선택 무기
2. **산탄총** - 근거리 확산 공격 (8발/32예비탄)
3. **기관단총(SMG)** - 높은 연사속도 (40발/120예비탄)
...
### 경제 & 진행 시스템
- **코인 시스템** - 초기 300 코인 지급, 20킬당 10 코인 획득
- **무기 상점** - 레벨 사이에 업그레이드 구매
- **레벨업 보너스** - 레벨당 예비 탄약 +30발 증가
...
### 보안 시스템
- 🔒 **개발자 도구 방지** - F12, Ctrl+Shift+I/J/C, Ctrl+U 차단
- 🛡️ **스크립트 변조 방지** - 코인/킬 수 무결성 체크 (1초마다)
- 🚫 **우클릭 방지** - 소스 보기 차단
- 🔍 **실시간 감지** - 비정상적인 값 변경 자동 복구
- ⚠️ **자동 보호** - 변조 감지 시 경고 및 게임 일시정지
...
│   │   └── Player.js     # 플레이어 캐릭터
│   ├── utils/            # 유틸리티 모듈
│   │   ├── security.js   # 보안 시스템 (F12 방지, 변조 감지)
│   │   ├── config.js     # 게임 설정
│   │   ├── ui.js         # UI 관리
